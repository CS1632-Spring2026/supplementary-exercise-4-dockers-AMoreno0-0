...
test.beforeEach(async ({page}) => {
    //TODO: Firefox browser version >= 105, or Chrome browser version >= 105 is installed and launched.
...
test('TEST-2-CATALOG', async({page}) => {
    ...
    await expect(page.getByAltText('Old Deuteronomy')).toHaveAttribute('src', '/images/cat2.jpg');
}); 
...
test('TEST-3-LISTING', async({page}) => {
    await page.getByRole('link', { name: 'Catalog' }).click();
    await expect(page.locator('#listing').getByRole('listitem')).toHaveCount(3);
    await expect (page.locator('//*[@id="cat-id3"]')).toContainText('ID 3. Mistoffelees');
});
...
test('TEST-5-RENT', async({page}) => {
    ...
    await expect(page.getByTestId('cat-id1')).toContainText("Rented out");
    await expect(page.getByTestId('cat-id2')).toContainText("ID 2. Old Deuteronomy");
    await expect(page.getByTestId('cat-id3')).toContainText("ID 3. Mistoffelees");
...
test('TEST-8-FEED', async({page}) => { 
    ...
    await expect(page.getByTestId('feedResult')).toHaveText("Nom, nom, nom.");
});
...
test('TEST-9-GREET-A-CAT', async({page}) => {
    ...
    await expect(page.getByRole('heading', { name: 'Meow!Meow!Meow!' })).toHaveText("Meow!Meow!Meow!");
});
...
test('TEST-10-GREET-A-CAT-WITH-NAME', async({page}) => {
    ...
    await expect(page.getByRole('heading', { name: 'Meow! from Jennyanydots.' })).toHaveText("Meow! from Jennyanydots.");
});
